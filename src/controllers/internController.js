...
try{
    ...
    const {name,email,mobile,collegeId}= data  //object destructing
...
const getInterns = async function(req, res){
    ...
    try {
        ...
        if (!shortName) {
            ...
            if (!interns.length > 0) {
                return res.status(400).send({ status: false, msg: "No Interns is applied for this internship" })
            }
            else {
...
//            res.status(400).send({status:false,msg:'enter data to get college detail'})
//            return
//        }


//      if(!isValid(name)){
//          res.status(400).send({status:false,msg:'enter college anabbreviation'})
//          return
//      }
// const internCollegeList = await internModel.find({name:name}).select({_id:1,email:1, name:1,mobile:1})
// if(!internCollegeList.length>0){
//     res.status(400).send({status:false,msg:'interns are not available'})
// }
// // const {name,collegeId}= collegeDetail


// res.status(201).send({status:true,data:internCollegeList})
